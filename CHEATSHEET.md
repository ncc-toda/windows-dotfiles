...
|---|---|
| **入れる**（学生） | `irm https://raw.githubusercontent.com/ncc-toda/windows-dotfiles/release/install.ps1 \| iex` |
| **dotfiles の中身を更新** | `just upgrade`（最新版 tarball を取り直して再適用） |
| **消す**（原状復帰） | `irm https://raw.githubusercontent.com/ncc-toda/windows-dotfiles/release/uninstall.ps1 \| iex` |

配布は固定ブランチ **`release`** から。`main` は開発版なので学生 URL には使わない。
