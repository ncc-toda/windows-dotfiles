...
tmux 風の Leader (`Ctrl+A`) プレフィックスは廃止。単発キーのみ:
...
セッション（タブ / ペイン / 開いていたパス）は 5 秒ごとに自動保存され、次回起動時に自動復元される（手動保存/復元キーは無し）。
...
- **仕組み**: 5 秒ごとに状態をスナップショット保存し、起動時に直近の状態を自動復元する。
  保存・復元は全自動で、手動キーは用意していない。
- **終了時には保存されない**。WezTerm に終了イベントが無く resurrect にも終了時保存が
  無いため、state を書くのは上記の定期タイマーだけ。最後の保存から終了までの
  変更は失われる（間隔を 5 秒に詰めてあるので実害は小さい）。60 秒だった頃は「タブを
  足してすぐ閉じる」が丸ごと消え、毎回同じ状態が復元され続ける状態だった。
- **定期保存のチェーンを間引いてはいけない**。`periodic_save` は自分を再登録し続ける
  チェーンで、設定は起動ごとに 2 回評価されるため 2 本張られる。だが 1 回目（mux 側）の
  タイマーは発火せず、実際に保存しているのは 2 回目（GUI 側）だけ。`wezterm.GLOBAL` で
  1 本に絞ると死んでいる方が残り、**保存が完全に止まる**（実測済み）。
