...
#
# WezTerm 設定の配置と、AutoHotkey スクリプト(Caps Lock 2度押しトグル /
# 左Shift=英数・右Shift=かな の IME 切替)の自動起動登録を
# `home-manager switch` (= just switch) に畳み込む。詳細は windows/README.md。
#
...
in
{
  home.activation.windowsSetup = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
    ...
    if [ -x "$PS" ] && [ -f "$BOOT" ] && [ -e "$WSLPATH" ]; then
      ...
      if [ -n "$WINBOOT" ]; then
        echo "windows: WezTerm 設定配置 + AHK 登録 (Caps Lock トグル / IME Shift切替) (bootstrap.ps1)"
